...
function openAdd(date=selectedDate,time=""){const serviceOpts=services.map(s=>`<option value="${s.id}">${esc(s.name)} · ${baht(s.price)} · ${s.duration||60} นาที</option>`).join("");openModal(`<h2>＋ เพิ่มคิว</h2><div class="form-grid"><label>ชื่อลูกค้า<input id="fName" placeholder="เช่น คุณเมย์"></label><label>เบอร์โทร<input id="fPhone" inputmode="tel" placeholder="08xxxxxxxx"></label><label>วันที่<input id="fDate" type="date" value="${date}"></label><label>เวลา<select id="fTime"><option value="">เลือกเวลา</option>${times.map(t=>`<option ${t===time?"selected":""}>${t}</option>`).join("")}</select></label><label>บริการ<select id="fService">${serviceOpts}</select></label><label>สถานะการชำระ<select id="fPay"><option value="unpaid">ยังไม่ชำระ</option><option value="paid">ชำระแล้ว</option></select></label><textarea id="fNote" placeholder="หมายเหตุ"></textarea></div><div class="modal-actions"><button class="primary" onclick="saveBooking()">ลงคิว</button></div>`)}
function findCustomer(name,phone){return customers.find(c=>(phone&&c.phone===phone)||c.name===name)}
async function saveBooking(){
 const name=$("fName").value.trim(),phone=$("fPhone").value.trim(),date=$("fDate").value,time=$("fTime").value,sid=$("fService").value,note=$("fNote").value.trim(),pay=$("fPay").value;
 ...
 const s=services.find(x=>String(x.id)===String(sid));const row={user_id:currentUser.id,customer_id:c.id,customer_name:name,phone,date,time,note,status:"waiting",payment_status:pay,service_id:s?.id==="local"?null:s?.id,service_name:s?.name||"ดูดวงทั่วไป",price:s?.price||300};
...
function openBooking(id){const b=bookings.find(x=>String(x.id)===String(id));if(!b)return;openModal(`<h2>🔮 รายละเอียดคิว</h2><div class="notice"><b>${esc(b.customer_name)}</b><br>${fmtDate(b.date)} · ${b.time}<br>${esc(serviceName(b))} · ${baht(priceOf(b))}</div><div class="form-grid"><label>สถานะ<select id="editStatus"><option value="waiting" ${b.status==="waiting"?"selected":""}>รอคิว</option><option value="doing" ${b.status==="doing"?"selected":""}>กำลังดูดวง</option><option value="done" ${b.status==="done"?"selected":""}>ดูเสร็จแล้ว</option><option value="cancelled" ${b.status==="cancelled"?"selected":""}>ยกเลิก</option></select></label><label>การชำระเงิน<select id="editPay"><option value="unpaid" ${b.payment_status!=="paid"?"selected":""}>ยังไม่ชำระ</option><option value="paid" ${b.payment_status==="paid"?"selected":""}>ชำระแล้ว</option></select></label><label>ราคา<input id="editPrice" type="number" value="${priceOf(b)}"></label><textarea id="editNote">${esc(b.note||"")}</textarea></div><div class="modal-actions"><button class="primary" onclick="updateBooking('${b.id}')">บันทึก</button><button class="danger" onclick="deleteBooking('${b.id}')">ลบคิว</button></div>`)}
async function updateBooking(id){const r=await supabaseClient.from("bookings").update({status:$("editStatus").value,payment_status:$("editPay").value,price:Number($("editPrice").value||0),note:$("editNote").value}).eq("id",id);if(r.error)return toast("แก้ไขไม่สำเร็จ: "+r.error.message);toast("บันทึกแล้ว ✓");closeModal();await loadData()}
